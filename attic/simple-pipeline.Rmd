...
The **PipeOp** does not work stand-alone, as we expect that it is wrapped in a `GraphNode`. This handles all connectivity to the previous and following nodes of the graph.
...
- The data transformation is learned and stored in the `params` slot of `op1`.
- The transformed task is saved in the `result` slot of `op1`.

```{r}
n1$is_learnt
scaled_task = n1$result
head(scaled_task$data())
```
