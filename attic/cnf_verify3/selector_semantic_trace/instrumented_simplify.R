...
simplify_cnf = function(entries, universe) {
  .trace_root = environment()
  trace_event(3L, environment(), 'entry')
  #################################
  # Some vocabulary:
  ...
  return_entries = function(entries) {
    trace_event(34L, environment(), 'return')
    structure(
...
trace_event(48L, environment())

  is_unit = lengths(entries) == 1L
trace_event(50L, environment())
  eliminated = logical(length(entries))
trace_event(51L, environment())
...
trace_event(103L, environment())
      eliminated[[unit_idx]] <<- TRUE
trace_event(104L, environment())
    }
...
trace_event(185L, environment())
          is_not_subset_of[[other_meta_idx]][meta_idx, symbol] <<- TRUE
trace_event(186L, environment())
...
trace_event(222L, environment())
  ...
  eliminate_symbol_from_clause = function(clause_idx, symbol) {
    clause = entries[[clause_idx]]
trace_event(237L, environment())
    clause[[symbol]] = NULL
trace_event(238L, environment())
...
trace_event(242L, environment())

    entries[[clause_idx]] <<- clause
trace_event(244L, environment())
    if (length(clause) == 1) {
      # new unit ahoy
...
trace_event(458L, environment())
  ...
  trace_event(467L, environment(), 'before')
...
trace_event(777L, environment())
      ...
      was_used[[hla_clause_idx]] = TRUE
      clause[[symbol]] = range_new
trace_event(784L, environment())
    }
  }

  trace_event(788L, environment(), 'before')
