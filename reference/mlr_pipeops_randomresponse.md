...
Takes in a
[`Prediction`](https://mlr3.mlr-org.com/reference/Prediction.html) of
`predict_type` `"prob"` (for
...
and generates a randomized `"response"` prediction.

For `"prob"`, the responses are sampled according to the probabilities
of the input
[`PredictionClassif`](https://mlr3.mlr-org.com/reference/PredictionClassif.html).
For `"se"`, responses are randomly drawn according to the `rdistfun`
parameter (default is [`rnorm`](https://rdrr.io/r/stats/Normal.html)) by
using the original responses of the input
[`PredictionRegr`](https://mlr3.mlr-org.com/reference/PredictionRegr.html)
as the mean and the original standard errors of the input
[`PredictionRegr`](https://mlr3.mlr-org.com/reference/PredictionRegr.html)
as the standard deviation (sampling is done observation-wise).
...
- packages :: `character`  
  Set of all required packages for the `private$.predict()` methods
  related to the `rdistfun` parameter. Default is `character(0)`.

## Input and Output Channels

`PipeOpRandomResponse` has one input channel named `"input"`, taking
`NULL` during training and a
[`Prediction`](https://mlr3.mlr-org.com/reference/Prediction.html)
during prediction.

`PipeOpRandomResponse` has one output channel named `"output"`,
producing `NULL` during training and a
[`Prediction`](https://mlr3.mlr-org.com/reference/Prediction.html) with
random responses during prediction.
...
- `rdistfun` :: `function`  
  A function for generating random responses when the predict type is
  `"se"`. This function must accept the arguments `n` (integerish number
  of responses), `mean` (`numeric` for the mean), and `sd` (`numeric`
  for the standard deviation), and must *vectorize* over `mean` and
  `sd`. Default is [`rnorm`](https://rdrr.io/r/stats/Normal.html).

## Internals

If the `predict_type` of the input
[`Prediction`](https://mlr3.mlr-org.com/reference/Prediction.html) does
not match `"prob"` or `"se"`, the input
[`Prediction`](https://mlr3.mlr-org.com/reference/Prediction.html) will
