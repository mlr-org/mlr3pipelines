...
Copies its input `outnum` times. This
...
followed by multiple different
[`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md)s.
However, when constructing big Graphs using the `%>>%`-operator,
`PipeOpCopy` can be helpful to specify which
[`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md) gets
connected to which.
...
`PipeOpCopy` has one input channel named `"input"`, taking any input
(`"*"`) both during training and prediction.

`PipeOpCopy` has multiple output channels depending on the `outnum`
...
R6-objects: If R6 objects are copied using `PipeOpCopy`, they must be
cloned beforehand.
