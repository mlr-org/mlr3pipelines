...
the minority class for nominal and continuous data using the SMOTENC
algorithm.

The algorithm generates for each minority instance a new data point
based on the `k` nearest neighbors of that data point. It treats integer
features as numeric. To not change feature types, the numeric, synthetic
data generated for these features are rounded back to integer. Because
of this, data generated through usage of this
[`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md) is not
exactly equal to data generated by calling
[`themis::smotenc`](https://themis.tidymodels.org/reference/smotenc.html)
directly on the same data set.

It can only be applied to [classification
tasks](https://mlr3.mlr-org.com/reference/TaskClassif.html) with factor
(or ordered) features and at least one numeric (or integer) feature that
...
- `k` :: `integer(1)`  
  Number of nearest neighbors used for generating new values from the
  minority class. Default is `5`.

- `over_ratio` :: `numeric(1)`  
  Ratio of the majority to minority class. Default is `1`. For details,
  see
  [`themis::smotenc`](https://themis.tidymodels.org/reference/smotenc.html).
