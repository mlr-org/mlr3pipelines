...
Subsamples a [`Task`](https://mlr3.mlr-org.com/reference/Task.html) to
use a fraction of the rows.

Sampling happens only during training phase. Subsampling a
[`Task`](https://mlr3.mlr-org.com/reference/Task.html) may be beneficial
for training time at possibly (depending on original
[`Task`](https://mlr3.mlr-org.com/reference/Task.html) size) negligible
...
removed rows according to the sampling. The output during prediction is
the unchanged input.
...
- `frac` :: `numeric(1)`  
  ...
  [`Task`](https://mlr3.mlr-org.com/reference/Task.html) to keep. May
  only be greater than 1 if `replace` is `TRUE`. Initialized to
  `(1 - exp(-1)) == 0.6321`.
...
- `stratify` :: `logical(1)`  
  ...
  input and if `use_groups = FALSE`.

- `use_groups` :: `logical(1)`  
  If `TRUE` and if the
  [`Task`](https://mlr3.mlr-org.com/reference/Task.html) has a column
  with role `group`, grouped observations are kept together during
  subsampling. In case of sampling with
...
Uses `task$filter()` to remove rows. If `replace` is `TRUE` and
identical rows are added, then the `task$row_roles$use` can *not* be
used to duplicate rows because of \[inaudible\]; instead the
`task$rbind()` function is used, and a new
