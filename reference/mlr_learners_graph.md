...
encapsulates a
[`Graph`](https://mlr3pipelines.mlr-org.com/reference/Graph.md) to be
used in [mlr3](https://mlr3.mlr-org.com/reference/mlr3-package.html)
...
- `graph` ::
  ...
  [`Graph`](https://mlr3pipelines.mlr-org.com/reference/Graph.md). This
  argument is usually cloned, unless `clone_graph` is `FALSE`; to access
  the [`Graph`](https://mlr3pipelines.mlr-org.com/reference/Graph.md)
...
- `task_type` :: `character(1)`  
  ...
  inferred for
  [`Graph`](https://mlr3pipelines.mlr-org.com/reference/Graph.md)s that
  are simple enough.
...
- `graph` ::
  ...
  is being wrapped. This field contains the prototype of the
  [`Graph`](https://mlr3pipelines.mlr-org.com/reference/Graph.md) that
  is being trained, but does *not* contain the model. Use `graph_model`
  to access the trained
  [`Graph`](https://mlr3pipelines.mlr-org.com/reference/Graph.md) after
  `$train()`. Read-only.

- `graph_model` ::
  [`Learner`](https://mlr3.mlr-org.com/reference/Learner.html)  
  [`Graph`](https://mlr3pipelines.mlr-org.com/reference/Graph.md) that
  is being wrapped. This
  [`Graph`](https://mlr3pipelines.mlr-org.com/reference/Graph.md)
...
- `pipeops` :: named `list` of
  [`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md)  
  Contains all
  [`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md)s in
  the underlying
  [`Graph`](https://mlr3pipelines.mlr-org.com/reference/Graph.md), named
  by the
...
- `pipeops_param_set` :: named
  ...
  all [`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md)s
  in the
  [`Graph`](https://mlr3pipelines.mlr-org.com/reference/Graph.md). See
  there for details.
...
- `plot(html = FALSE, horizontal = FALSE)`  
  ...
  Plot the
  [`Graph`](https://mlr3pipelines.mlr-org.com/reference/Graph.md), using
  either the [igraph](https://CRAN.R-project.org/package=igraph) package
  (for `html = FALSE`, default) or the `visNetwork` package for
...
- `base_learner(recursive = Inf, return_po = FALSE, return_all = FALSE, resolve_branching = TRUE)`  
  ...
  [`Graph`](https://mlr3pipelines.mlr-org.com/reference/Graph.md) is
  ...
  [`PipeOpBranch`](https://mlr3pipelines.mlr-org.com/reference/mlr_pipeops_branch.md)s,
...
[`Graph`](https://mlr3pipelines.mlr-org.com/reference/Graph.md)s that do
not modify features too much, but may give unexpected results for
`Graph`s that add new features or move information between features.

As an example, consider a feature `A` with missing values, and a feature
`B` that is used for imputation, using a
[`po("imputelearner")`](https://mlr3pipelines.mlr-org.com/reference/mlr_pipeops_imputelearner.md).
...
[`Graph`](https://mlr3pipelines.mlr-org.com/reference/Graph.md) is known
to not have an impact on the relevant properties.
...
[`Graph`](https://mlr3pipelines.mlr-org.com/reference/Graph.md) via
[`gunion()`](https://mlr3pipelines.mlr-org.com/reference/gunion.md);
however, this will usually not result in a valid
[`Graph`](https://mlr3pipelines.mlr-org.com/reference/Graph.md) that can
work as a [`Learner`](https://mlr3.mlr-org.com/reference/Learner.html).
`graph` can furthermore be a
[`Learner`](https://mlr3.mlr-org.com/reference/Learner.html), which is
then automatically wrapped in a
[`Graph`](https://mlr3pipelines.mlr-org.com/reference/Graph.md), which
is then again wrapped in a `GraphLearner` object; this usually only adds
