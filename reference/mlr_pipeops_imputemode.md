...
features missing values imputed by (column-wise) mode.
...
The `$state$model` is a named `list` of a vector of length one of the
type of the feature, indicating the mode of the respective feature.
...
Note that every random imputation is drawn independently, so different
values may be imputed if multiple values are missing.
