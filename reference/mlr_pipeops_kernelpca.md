...
Extracts kernel principal components from data. Only affects numerical
features. See [kernlab::kpca](https://rdrr.io/pkg/kernlab/man/kpca.html)
for details.
...
numeric parameters replaced by their principal components.

## State

The `$state` is a named `list` with the `$state` elements inherited from
[`PipeOpTaskPreproc`](https://mlr3pipelines.mlr-org.com/reference/PipeOpTaskPreproc.md),
as well as the returned [`S4`](https://rdrr.io/r/base/isS4.html) object
of the function
[`kernlab::kpca()`](https://rdrr.io/pkg/kernlab/man/kpca.html).

The `@rotated` slot of the `"kpca"` object is overwritten with an empty
matrix for memory efficiency.

The slots of the [`S4`](https://rdrr.io/r/base/isS4.html) object can be
accessed by accessor function. See
[kernlab::kpca](https://rdrr.io/pkg/kernlab/man/kpca.html).
...
- `kernel` :: `character(1)`  
  The standard deviations of the principal components. See
  [`kpca()`](https://rdrr.io/pkg/kernlab/man/kpca.html).

- `kpar` :: `list`  
  List of hyper-parameters that are used with the kernel function. See
  [`kpca()`](https://rdrr.io/pkg/kernlab/man/kpca.html).

- `features` :: `numeric(1)`  
  Number of principal components to return. Default 0 means that all
  principal components are returned. See
  [`kpca()`](https://rdrr.io/pkg/kernlab/man/kpca.html).

- `th` :: `numeric(1)`  
  The value of eigenvalue under which principal components are ignored.
  Default is 0.0001. See
  [`kpca()`](https://rdrr.io/pkg/kernlab/man/kpca.html).

- `na.action` :: `function`  
  Function to specify NA action. Default is
  [`na.omit`](https://rdrr.io/r/stats/na.fail.html). See
  [`kpca()`](https://rdrr.io/pkg/kernlab/man/kpca.html).

## Internals

Uses the [`kpca()`](https://rdrr.io/pkg/kernlab/man/kpca.html) function.
