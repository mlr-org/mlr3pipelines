...
[`Graph`](https://mlr3pipelines.mlr-org.com/reference/Graph.md) is a
`Multiplicity`-object, then the
[`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md) that
receives this object will *usually* be called once for each contained
value. The result of each of these calls is then, again, packed in a
`Multiplicity` and sent along the outgoing edge(s) of that
[`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md). This
means that a `Multiplicity` can cause multiple
[`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md)s in a
row to be run multiple times, where the run for each element of the
`Multiplicity` is independent from the others.

Most [`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md)s
...
are "aware" of `Multiplicity` objects. These may either *create* a
...
[`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md)s to be
run multiple times – or *collect* a `Multiplicity`, being called only
once even though their input is a `Multiplicity` (e.g.
...
called variably many times if "sandwiched" between `Multiplicity`
...
will usually collect (input channel) or create (output channel) a
`Multiplicity`.
[`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md)s
...
If a [`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md)
is trained on a `Multiplicity`, the `$state` slot is set to a
`Multiplicity` as well; this `Multiplicity` contains the "original"
`$state` resulting from each individual call of the
[`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md) with
the input `Multiplicity`'s content. If a
[`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md) was
trained with a `Multiplicity`, then the
