...
Normalizes the data row-wise. This is a natural generalization of the
"sign" function to higher dimensions.
...
numeric features replaced by their normalized versions.
...
- `norm` :: `numeric(1)`  
  Norm to use. Rows are scaled to `sum(x^norm)^(1/norm) == length` for
  finite `norm`, or to `max(abs(x)) == length` if `norm` is `Inf`.
  Default is 2.
