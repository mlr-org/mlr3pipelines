...
[`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md)s for
...
[`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md)s, and
a [`data.table`](https://rdrr.io/pkg/data.table/man/data.table.html) of
edges. Both for training and prediction, the `Graph` performs
topological sorting of the
[`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md)s and
executes their respective `$train()` or `$predict()` functions in order,
moving the
...
- `edges` ::
  ...
  [`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md)s. A
  [`data.table`](https://rdrr.io/pkg/data.table/man/data.table.html).
...
- `input` ::
  ...
  [`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md)
  `$id` of the
  [`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md) the
  channel pertains to, and channel name as the
  [`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md)
  knows it.
...
- `add_pipeop(op, clone = TRUE)`  
  ...
  the `Graph`. This does not add any edges, so the new
  [`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md) will
  not be connected within the `Graph` at first.  
  Instead of supplying a
  [`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md)
  directly, an object that can naturally be converted to a
  [`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md) can
  also be supplied, e.g. a
...
- `add_edge(src_id, dst_id, src_channel = NULL, dst_channel = NULL)`  
  ...
  [`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md) have
  only one input / output channel and `src_channel` / `dst_channel` are
  therefore unambiguous, they can be omitted (i.e. left as `NULL`).

- `chain(gs, clone = TRUE)`  
  (`list` of `Graph`s, `logical(1)`) -\> `self`  
  Takes a list of `Graph`s or
  [`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md)s (or
  objects that can be automatically converted into `Graph`s or
  [`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md)s,
  see
...
- `print(dot = FALSE, dotname = "dot", fontsize = 24L)`  
  ...
  [`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md) and
  columns `ID` (`$id` of `PipeOp`), `State` (short representation of
  `$state` of
  [`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md)),
  `sccssors`
  ([`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md)s
  that take their input directly from the
  [`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md) on
  this line), and `prdcssors` (the
  [`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md)s
  that produce the data that is read as input by the
  [`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md) on
  this line). If `dot` is `TRUE`, print a DOT representation of the
...
- `set_names(old, new)`  
  ...
  identified by `old` to the corresponding item in `new`. This should be
  used instead of changing a
  [`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md)'s
  `$id` value directly!
...
- `train(input, single_input = TRUE)`  
  ...
  is dependent on the
  [`PipeOp`](https://mlr3pipelines.mlr-org.com/reference/PipeOp.md)s in
  the `Graph`. If `single_input` is `FALSE`, then `input` should be a
