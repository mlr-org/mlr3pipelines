...
Handles target transformation operations that do not need explicit
inversion. In case the new target is required during predict, creates a
vector of `NA`. Works similar to
...
but forgoes the inversion step. In case target after the `trafo` is a
factor, levels are saved to `$state`.  

During prediction: Sets all target values to `NA` before calling the
`trafo` again. In case target after the `trafo` is a factor, levels
saved in the `state` are set during prediction.

As a special case when `trafo` is `identity` and `new_target_name`
matches an existing column name of the data of the input
[`Task`](https://mlr3.mlr-org.com/reference/Task.html), this column is
set as the new target. Depending on `drop_original_target` the original
target is then either dropped or added to the features.
...
- `trafo` :: `function`  
  ...
  the target, i.e., taking a single argument. Default is `identity`.
  Note, that the data passed on to the target is a `data.table`
  consisting of all target column.

- `new_target_name` :: `character(1)`  
  Optionally give the transformed target a new name. By default the
...
[`list()`](https://rdrr.io/r/base/list.html) if none of the targets have
levels.
