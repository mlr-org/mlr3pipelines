...
Adds features according to expressions given as formulas that may depend
on values of other features. This can add new features, or can change
existing features.
...
[`Task`](https://mlr3.mlr-org.com/reference/Task.html) with added and/or
mutated features according to the `mutation` parameter.
...
- `mutation` :: named `list` of `formula`  
  Expressions for new features to create (or present features to
  change), in the form of `formula`. Each element of the list is a
  `formula` with the name of the element naming the feature to create or
  change, and the formula expression determining the result. This
  expression may reference other features, as well as variables visible
  at the creation of the `formula` (see examples). Initialized to
  [`list()`](https://rdrr.io/r/base/list.html).
...
A `formula` created using the `~` operator always contains a reference
to the `environment` in which the `formula` is created. This makes it
possible to use variables in the `~`-expressions that both reference
either column names or variable names.
...
previous formula. However, if existing features are changed, precedence
is given to the original ones before the newly constructed ones.
