...
Chunks its input into `outnum` chunks. Creates `outnum`
[`Task`](https://mlr3.mlr-org.com/reference/Task.html)s during training,
and simply passes on the input during `outnum` times during prediction.
...
`PipeOpChunk` has one input channel named `"input"`, taking a
[`Task`](https://mlr3.mlr-org.com/reference/Task.html) both during
training and prediction.

`PipeOpChunk` has multiple output channels depending on the `options`
construction argument, named `"output1"`, `"output2"`, ... All output
channels produce (respectively disjoint, random) subsets of the input
[`Task`](https://mlr3.mlr-org.com/reference/Task.html) during training,
and pass on the original
...
Uses the
[`mlr3misc::chunk_vector()`](https://mlr3misc.mlr-org.com/reference/chunk_vector.html)
function.
