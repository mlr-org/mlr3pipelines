...
Using `PipeOpLearnerPICVPlus`, it is possible to embed a
[`mlr3::Learner`](https://mlr3.mlr-org.com/reference/Learner.html) into
a [`Graph`](https://mlr3pipelines.mlr-org.com/dev/reference/Graph.md).
`PipeOpLearnerPICVPlus` can then be used to perform cross validation
plus (or jackknife plus). During training, `PipeOpLearnerPICVPlus`
performs cross validation on the training data. During prediction, the
models from the training stage are used to construct predictive
confidence intervals for the prediction data based on out-of-fold
residuals and out-of-fold predictions.
...
- `learner` ::
  ...
  [`LearnerRegr`](https://mlr3.mlr-org.com/reference/LearnerRegr.html)
  to use for the cross validation models in the Cross Validation Plus
  method. This argument is always cloned; to access the
  [`Learner`](https://mlr3.mlr-org.com/reference/Learner.html) inside
  `PipeOpLearnerPICVPlus` by-reference, use `$learner`.  
...
- `param_vals` :: named `list`  
  ...
  settings that would otherwise be set during construction. Default is
  [`list()`](https://rdrr.io/r/base/list.html).

## Input and Output Channels

`PipeOpLearnerPICVPlus` has one input channel named `"input"`, taking a
...
`PipeOpLearnerPICVPlus` has one output channel named `"output"`,
...
[`PredictionRegr`](https://mlr3.mlr-org.com/reference/PredictionRegr.html)
with `predict_type` `quantiles` on the prediction input data. The
`alpha` and `1 - alpha` quantiles are the `quantiles` of the prediction
interval produced by the cross validation plus method. The `response` is
the median of the prediction of all cross validation models on the
prediction data.
...
- `cv_model_states` :: `list`  
  List of the state of each cross validation model created by the
  [`Learner`](https://mlr3.mlr-org.com/reference/Learner.html)'s
  `$.train()` function during resampling with method `"cv"`.

- `residuals` :: `data.table`  
  `data.table` with columns `fold` and `residual`. Lists the Regression
  residuals for each observation and cross validation fold.
...
this object, as well as:

- `folds` :: `numeric(1)`  
  Number of cross validation folds. Initialized to 3.

- `alpha` :: `numeric(1)`  
  Quantile to use for the cross validation plus prediction intervals.
  Initialized to 0.05.
...
- `learner_model` ::
  [`Learner`](https://mlr3.mlr-org.com/reference/Learner.html) or
  `list`  
  If the `PipeOpLearnerPICVPlus` has been trained, this is a `list`
  containing the
  [`Learner`](https://mlr3.mlr-org.com/reference/Learner.html)s of the
  cross validation models. Otherwise, this contains the
  [`Learner`](https://mlr3.mlr-org.com/reference/Learner.html) that is
  being wrapped. Read-only.

- `predict_type`  
  Predict type of the `PipeOpLearnerPICVPlus`, which is always
  `"response" "quantiles"`. This can be different to the predict type of
  the [`Learner`](https://mlr3.mlr-org.com/reference/Learner.html) that
  is being wrapped.
...
Barber RF, Candes EJ, Ramdasa A, Tibshirani RJ (2021). “Predictive
inference with the jackknife+.” *Annals of Statistics*, **49**, 486–507.
...
#>        1  21.0 23.51111 11.556667 33.07500
#>        2  21.0 23.51111 11.556667 33.07500
#>        3  22.8 25.56667 16.252222 34.06875
#>      ---   ---      ---       ---      ---
#>       30  19.7 23.51111 11.556667 33.07500
#>       31  15.0 15.66667  7.459701 24.41109
#>       32  21.4 25.56667 16.252222 34.06875
#> 
```
