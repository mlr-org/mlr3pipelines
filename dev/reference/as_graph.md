...
[`Graph`](https://mlr3pipelines.mlr-org.com/dev/reference/Graph.md) if
...
- apply
  ...
  to `x` if it is a `list`, which recursively applies `as_graph()` to
  all list elements first

- create a
  [`Graph`](https://mlr3pipelines.mlr-org.com/dev/reference/Graph.md)
  with only one element if `x` is a
  [`PipeOp`](https://mlr3pipelines.mlr-org.com/dev/reference/PipeOp.md)
  or can be converted to one using
...
[`Graph`](https://mlr3pipelines.mlr-org.com/dev/reference/Graph.md) `x`
