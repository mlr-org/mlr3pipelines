...
Linearly transforms numeric data columns so they are between `lower` and
`upper`. The formula for this is \\x' = offset + x \* scale\\, where
\\scale\\ is \\(upper - lower) / (max(x) - min(x))\\ and \\offset\\ is
\\-min(x) \* scale + lower\\. The same transformation is applied during
...
[`PipeOpTaskPreproc`](https://mlr3pipelines.mlr-org.com/dev/reference/PipeOpTaskPreproc.md),
as well as the two transformation parameters \\scale\\ and \\offset\\
