...
Inverts target-transformations done during training based on a supplied
inversion function. Typically should be used in combination with a
subclass of
[`PipeOpTargetTrafo`](https://mlr3pipelines.mlr-org.com/dev/reference/PipeOpTargetTrafo.md).

During prediction phase the function supplied through `"fun"` is called
with a `list` containing the `"prediction"` as a single element, and
should return a `list` with a single element (a
[`Prediction`](https://mlr3.mlr-org.com/reference/Prediction.html)) that
is returned by `PipeOpTargetInvert`.
...
`PipeOpTargetInvert` has two input channels named `"fun"` and
`"prediction"`. During training, both take `NULL` as input. During
prediction, `"fun"` takes a function and `"prediction"` takes a
[`Prediction`](https://mlr3.mlr-org.com/reference/Prediction.html).

`PipeOpTargetInvert` has one output channel named `"output"` and returns
...
Should be used in combination with a subclass of
[`PipeOpTargetTrafo`](https://mlr3pipelines.mlr-org.com/dev/reference/PipeOpTargetTrafo.md).
...
#> }
#> <bytecode: 0x559e99269eb0>
#> <environment: 0x559e9926a770>
#> 
#> $output
#> 
#> ── <TaskRegr> (506x18): Boston Housing Prices ──────────────────────────────────
