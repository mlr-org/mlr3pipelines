...
[`Task`](https://mlr3.mlr-org.com/reference/Task.html) with the original
affected columns replaced by the columns created by applying
`applicator` to each row.
...
- `applicator` :: `function`  
  Function to apply to each row in the affected columns of the task. The
  return value should be a vector of the same length for every input.
  Initialized as [`identity()`](https://rdrr.io/r/base/identity.html).

- `col_prefix` :: `character(1)`  
  If specified, prefix to be prepended to the column names of affected
  columns, separated by a dot (`.`). Initialized as `""`.

## Internals

Calls [`apply`](https://rdrr.io/r/base/apply.html) on the data, using
the value of `applicator` as `FUN`.
