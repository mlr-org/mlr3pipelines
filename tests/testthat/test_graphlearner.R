...
test_that("basic graphlearn tests", {
  ...
  dblrn$param_set$param_vals$save_tasks = TRUE
...
test_that("graphlearner parameters behave as they should", {
  ...
  dbgr$param_set$param_vals$debug.x = 1

  expect_equal(dbgr$param_set$param_vals$debug.x, 1)
  expect_equal(dbgr$pipeops$debug$param_set$param_vals$x, 1)
  expect_equal(dbgr$pipeops$debug$learner$param_set$param_vals$x, 1)

  dbgr$pipeops$debug$param_set$param_vals$x = 0

  expect_equal(dbgr$param_set$param_vals$debug.x, 0)
  expect_equal(dbgr$pipeops$debug$param_set$param_vals$x, 0)
  expect_equal(dbgr$pipeops$debug$learner$param_set$param_vals$x, 0)

  dbgr$pipeops$debug$learner$param_set$param_vals$x = 0.5

  expect_equal(dbgr$param_set$param_vals$debug.x, 0.5)
  expect_equal(dbgr$pipeops$debug$param_set$param_vals$x, 0.5)
  expect_equal(dbgr$pipeops$debug$learner$param_set$param_vals$x, 0.5)

  expect_error({dbgr$param_set$param_vals$debug.x = "a"})
  expect_error({dbgr$pipeops$debug$param_set$param_vals$x = "a"})
  expect_error({dbgr$pipeops$debug$learner$param_set$param_vals$x = "a"})
