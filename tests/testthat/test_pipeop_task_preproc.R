...
test_that("PipeOpTaskPreproc - affect_columns and selectors behavior", {
  ...
  expect_equal_data_table(op$train(list(task))[[1]]$data(), dt, ignore_col_order = TRUE)  # task$cbind does not return same column order
  expect_set_equal(op$state$affected_cols, c("num1", "num2", "chr", "lgl"))
  expect_equal_data_table(op$train(list(task))[[1]]$data(), dt, ignore_col_order = TRUE)
