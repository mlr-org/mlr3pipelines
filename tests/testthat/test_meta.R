...
test_that("expect_equal_r6 ignores active bindings and their label cache", {
  evaluations = new.env(parent = emptyenv())
  evaluations$count = 0L
  ActiveBindingClass = R6::R6Class(
    "ActiveBindingClass",
    public = list(
      value = 1,
      initialize = function(cached_label = NULL) private$.label = cached_label
    ),
    private = list(.label = NULL),
    active = list(label = function(value) {
      if (!missing(value)) stop("label is read-only")
      evaluations$count = evaluations$count + 1L
      if (is.null(private$.label)) private$.label = "cached label"
      private$.label
    })
  )
  object = ActiveBindingClass$new()
  expected = ActiveBindingClass$new(cached_label = "cached label")

  expect_success(expect_equal_r6(object, expected))
  expect_equal(evaluations$count, 0L)
