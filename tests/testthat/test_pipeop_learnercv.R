...
test_that("PipeOpLearnerCV - param values", {
  ...
  expect_equal(polrn$param_vals, list(resampling = "cv", folds = 3, keep_response = FALSE))
  polrn$param_vals$minsplit = 2
  expect_equal(polrn$param_vals, list(minsplit = 2, resampling = "cv", folds = 3, keep_response = FALSE))
  polrn$param_vals$folds = 4
  expect_equal(polrn$param_vals, list(minsplit = 2, resampling = "cv", folds = 4, keep_response = FALSE))
})
