...
compute_cyclic_date_features = function(date_features, features, date_var) {
  do.call(c, args = lapply(features, FUN = function(feature) {
    ...
    value = if (feature %in% c("month", "week_of_year", "day_of_year", "day_of_month")) {
      date_features[[paste0(date_var, ".", feature)]] - 1L
    } else {
      date_features[[paste0(date_var, ".", feature)]]
    }
    maximum = switch(feature,
      month = 12L,
      week_of_year = 52L,
      day_of_year = (365L + as.integer((date_features[[paste0(date_var, ".", "year")]] %% 400L == 0L) |
        (date_features[[paste0(date_var, ".", "year")]] %% 4L == 0L &
        date_features[[paste0(date_var, ".", "year")]] %% 100L != 0L))),
      day_of_month = get_days_per_month(date_features[[paste0(date_var, ".", "year")]],
        month = date_features[[paste0(date_var, ".", "month")]]),
      day_of_week = 7L,
      hour = 24L,
      minute = 60L,
      second = 60L)
