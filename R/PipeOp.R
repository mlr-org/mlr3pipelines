...
#' for each channel against `$input` and then passed on to the `$.train()` or `$.predict()` functions. There the data is processed and
...
#'
#' @section Internals:
#' [`PipeOp`] is an abstract class with abstract functions `$.train()` and `$.predict()`. To create a functional
...
#' channels in `$output`. The `$.train()` and `$.predict()` function should not be called by the user;
...
#'   (or explicitly attached using `require()`) in `$.train()` *and* `$.predict()`, but
...
#'   `$train()`, because `$.train()` may theoretically be executed in a different `R`-session (e.g. for parallelization).
#' * input :: [`data.table`] with columns `name` (`character`), `train` (`character`), `predict` (`character`)\cr
...
#'   are checked by the [`PipeOp`] itself and do not need to be checked by `$.train()` / `$.predict()` code.\cr
...
#'   It's an Abstract function that must be implemented by concrete subclasses. `$.train()` is called by `$train()` after
...
#'   The `$.train()` method should not be called by a user; instead, the `$train()` method should be used which does some
...
#'   Abstract function that must be implemented by concrete subclasses. `$.predict()` is called by `$predict()` after
#'   typechecking and works analogously to `$.train()`. Unlike `$.train()`, `$.predict()` should not modify
#'   the [`PipeOp`] in any way.\cr
#'   Just as `$.train()`, `$.predict()` should not be called by a user; instead, the `$predict()` method should be used.
#' * `print()` \cr
#'   () -> `NULL` \cr
