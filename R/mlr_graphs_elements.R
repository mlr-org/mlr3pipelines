...
#'   Optional id prefix to prepend to [`PipeOpTargetInvert`] ID. The resulting ID will be
...
pipeline_targettrafo = function(graph, trafo_pipeop = PipeOpTargetMutate$new(), id_prefix = "") {
  ...
  graph$add_edge(src_id = trafo_pipeop$id, dst_id = target_invert_id, src_channel = 1L, dst_channel = 1L)
  graph$add_edge(src_id = trafo_pipeop$id, dst_id = ids[1L], src_channel = 2L, dst_channel = 1L)
  graph$add_edge(src_id = ids[length(ids)], dst_id = target_invert_id, src_channel = 1L, dst_channel = 2L)
