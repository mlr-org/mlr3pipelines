...
[Graph], [Graph] -> [Graph]
If G or H are PipeOps, they will be leveled up to a Graph, containing a single node.
...
Case n-1: If length(rhs(G)) == n and length(lhs(H)) == 1, so G has multiple sinks, and H a single source, we connect all nodes in rhs(G) with the node lhs(H).
...
FIXME: we need to specify the concept of branching / multiplexing
