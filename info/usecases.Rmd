...
##### [[PipeOpFilter]]
  ...
  - **train**:
    - input: [[Task]]
    - does: Filters features according to mlr3filters. Featsel class is given as ParamVal. Selected features saved in 'state'
    - returns: [[Task]]
...
We use the `PipeOpDownSample` operator in conjunction with a `PipeOpLearner` to train a model. `pipeline_greplicate()` let's us do the same operation multiple times.
