...
- CNF bug 19 is fixed in `9fb17923`: roxygen2 `@rawNamespace` tags register `cnf_and`/`cnf_or` directly for all three classes. Distinct wrappers still conflict, and even aliases can conflict again after separate byte-compilation on R 3.6. Repeated `@exportS3Method` tags do not generate the required third `S3method()` argument. See `attic/cnf_fixes/ops_dispatch/README.md` for compiled-package validation and the preserved prototype.
- Current CNF repair status is in `attic/cnf_fixes/README.md`, using the user's stable bug numbers. The 2026-09-07 follow-ups fix 1, 2, 7, 8, 11, 12, 13, 14, 15, 16, and 19; only 3, 4, 5, 6, 9, 10, 17, and 18 remain open. Campaign notes below describe the earlier baseline for repaired input/comparison issues. Some attic reproductions assert the old failures; use `tests/testthat/test_Cnf{Inputs,Selectors,Constructors,Comparisons,Operators}.R` for the repaired behavior.
...
- CNF binary operators now use registrations from NAMESPACE, so simply sourcing the six R files does not register them. Use an installed/development package namespace for operator checks; standalone harnesses must explicitly process the registrations. The old internal binary method names such as `&.CnfFormula` were removed in `9fb17923`; `getS3method("&", "CnfFormula")` retrieves the registered shared handler. Historical scripts using old names should run against their recorded baseline.
