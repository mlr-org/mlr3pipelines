...
- A backward-compatible route for CNF bug 19 is recorded in `attic/cnf_fixes/ops_dispatch/README.md`: register one shared compiled handler per operator for all three classes using the third argument of `S3method()`. Distinct wrappers still conflict, and even aliases can conflict again after separate byte-compilation on R 3.6. The prototype passes 242 operator/truth/class cases on R 3.6 and R 4.6; it is not yet a production repair.
