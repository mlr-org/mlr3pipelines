...
Computational results in that graph only flow forward, but we store connection to predecessors of a node for
convenience.
...
# FIXME: results in pipeop or in graphnode
...
- get_node [character(1)] -> [GraphNode]
- get_pipeop [character(1)] -> [PipeOp]
- ids [character]
...
Connection operator, connects 2 partial graphs.
...
Case 1-1: If length(rhs(G)) == 1 and length(lhs(H)), so G has a single sink and H has a single source, we directly connect them.
...
Its specific aggregtaion functionality handles this list (typically
by somehow combinining the elements to a larger object, or selecting from the input elelemnts).
Examples of aggregators are PipeOpModelAvg, PipeOpFeatureUnion, PipeOpUnbranch.

An broadcasting PipeOp always returns a list. If the PipeOp is used in a graph,
...
length as its encapsulating GraphNode has successors.
Examples of broadcasters are PipeOpCopy, PipeOpBranch.

# Topological sorting and layer of a Graph
A DAG is made up of layers of nodes, depending on how "late" in the DAG a node appears, i.e., many how previous
computational steps need to be processed until we can finally compute that node. To make this more
explicit: We call the set of all nodes with no incoming endges the set of "sources" of a graph, or the
...
When we train computational graph, we proceed in topological layers. We initialize by
feeding the input object (a task) into each node in layer 0. We then compute all results
in layer 0, store the respectice results on each node. Then we train layer 1. This is possible,
as all nodes in layer only require results from layer 0. Then we train layer 2. This is possible
as all nodes in layer 2 only require results from layer 1 or 2. And so on.
