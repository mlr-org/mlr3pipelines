...
Then new `PipeOp`s are added to the `pipeop` slot using `add_pipeop()`.
...
This can be done using `PipeOpNull`, which simply passes on its inputs unchanged, both during the training and the predict phase.
