...
`mlr3pipelines` uses the [`r ref_pkg("paradox")`](https://paradox.mlr-org.com) package to define parameter spaces for `PipeOp`s.
...
The `Graph`s, when encapsulated in `GraphLearner`s, can even be tuned using the tuning functionality in `r mlr3tuning`.
...
The `param_set` must be a `ParamSet` from the `r ref_pkg("paradox")` package; see [the tuning chapter](#searchspace) or @sec-paradox for more information on how to define parameter spaces.
...
This can easily be done when using the `r mlr3misc` library's `mlr3misc::invoke()` function, which has functionality similar to `"do.call()"`.
